		...
		if (USBi_URI & intr) {
			...
			if (udc->gadget.ats_reset_irq_count == 50) {
				...
				if (udc->driver->broadcast_abnormal_usb_reset) {
					...
					/*
					 * FIXME: A short-term workaround for switch board issue.
					 * It can reduce the rate of ramdump, because sending uevent
					 * in interrupt will spend too long time.
					 * The execution time of interrupt will be as much as possible 
					 * smaller.
					 */
